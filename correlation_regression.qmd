...
## Regression with one feature {#sec-regression}
...
```{r plot-trump-police-regression-line}
df_anes |>
  ggplot(aes(x = therm_police, y = therm_trump)) +
...
```{r calculate-correlation-and-rsquared}
...
```{r create-education-binary-example}
#| echo: false
df_fake_educ <- tibble(
...
```{r plot-binary-response-regression}
df_anes |>
...
```{r manual-regression-calculation}
