...
When you render the document, the code you put into each [code chunk]{.concept} will be run in R, and the output will be included in the document.
...
`simple-arithmetic` is the label that I chose for this code block.
Chunk labels aren't necessary --- you can just write `{r}` instead of `{r code-chunk-label}` at the start of the chunk --- but they can make it easier to debug if your Quarto file isn't rendering correctly.

If you don't want to remember this goofy syntax yourself, you can run `Code -> Insert Chunk` from the RStudio menu.
Or even better, for the efficiency aficionados, hit `Ctrl-Alt-I` (Windows) or `Command-Option-I` (Mac) to insert a code chunk directly into your Quarto file.
...
```{r calculate-turnout-percentage}
